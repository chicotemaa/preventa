...
export function calculateUnitPricing(
  ...
  const packageQuantity = detectPackageQuantity(text, price);

  if (packageQuantity && packageQuantity > 1) {
    return {
      comparisonPrice: roundMoney(price / packageQuantity),
      packageQuantity,
      packageLabel: `pack x ${packageQuantity}`,
    };
  }
...
function detectPackageQuantity(text: string, price: number) {
  const normalizedText = normalizePackagingText(text);
  const presentation = extractProductPresentation(text);

  if (
    isValidPackageQuantity(presentation.packageCount) &&
    shouldUsePresentationPackageQuantity(
      normalizedText,
      presentation.packageCount,
      presentation.hasPowderSignal,
      price,
    )
  ) {
    return presentation.packageCount;
  }

  const unitWord = "(?:unid(?:ad)?(?:es)?|uds?|u)";
  const patterns = [
